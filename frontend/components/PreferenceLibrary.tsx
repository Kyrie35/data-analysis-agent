...
type PreferenceLibraryProps = {
  ...
  /** drawer: 侧滑；page: 嵌入页面内容（无大标题时配合外层页签） */
  variant?: "drawer" | "page";
  /** 表报偏好 / 取数偏好，默认表报 */
  scope?: PreferenceScope;
  /** page 模式下是否显示内置大标题；偏好库总页用页签时建议 false */
...
export default function PreferenceLibrary({
  ...
  const scopeMeta = PREFERENCE_SCOPE_META[scope];
  ...
  useEffect(() => {
    ...
    setItems(loadPreferences(scope));
    setGroups(loadGroups(scope));
    setTitleQuery("");
    setGroupFilter("all");
    setEditingId(null);
    setDraft(emptyDraft);
  ...
  function deleteGroup(id: string) {
    ...
    saveGroups(nextGroups, scope);
    savePreferences(nextItems, scope);
  ...
  const body = (
    <div className={`flex-1 space-y-4 overflow-y-auto ${isPage ? "" : "px-5 py-4"}`}>
      {error && (
      ...
      {editingId ? (
        ...
        <button
          type="button"
          onClick={startCreate}
          className="w-full rounded-xl border border-dashed border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700 hover:border-blue-400 hover:text-blue-700"
        >
          + 新增偏好
        </button>
      )}
  ...
  if (isPage) {
    return (
      <div>
        {showPageHeader && (
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-slate-900">{scopeMeta.label}</h1>
            <p className="mt-2 text-sm text-slate-500">{scopeMeta.description}</p>
          </div>
        )}
        {!showPageHeader && (
          <p className="mb-4 text-sm text-slate-500">{scopeMeta.description}</p>
        )}
        {body}
      </div>
    );
      ...
      <aside className="flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{scopeMeta.description}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
